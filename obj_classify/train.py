...
#ラベル付けした学習データのテキストファイルを指定
flags.DEFINE_string('train', 'train.txt', 'File name of train data')

#ラベル付けしたテストデータのテキストファイルを指定
flags.DEFINE_string('test', 'train.txt', 'File name of train data')

#学習用の画像データの入っているフォルダを指定
flags.DEFINE_string('train_image_dir', 'train_image', 'Directory of images')

#テスト用の画像データの入っているフォルダを指定
flags.DEFINE_string('test_image_dir', 'train_image', 'Directory of images')
...
if __name__ == '__main__':
	...
	for line in f:
		...
		#画像を読み込む
		#print(FLAGS.train_image_dir + '/' + l[0])
		img = cv2.imread(FLAGS.train_image_dir + '/' + l[0])
		
		#cv2.imshow("image", img)
		#cv2.waitKey(0)
		
		#読み込んだ画像を224*224にリサイズ
		#リサイズのサイズを224*224にするとmemoryerrorになる（スペック不足？）
		#img = cv2.resize(img, (224, 224))
		img = cv2.resize(img, (28, 28))
		#i += 1
		
		#一列にしたあと、0-1のfloat型にする
	...
	#numpy形式に変換 MemoryErrorになる場合はリサイズのサイズを小さくする
	...
	for line in f:
		...
		#画像を読み込む
		#print(FLAGS.train_image_dir + '/' + l[0])
		img = cv2.imread(FLAGS.test_image_dir + '/' + l[0])
	...
	with tf.Graph().as_default():
		...
		for step in range(FLAGS.max_steps):
			...
			for i in range(int(len(train_image)/batch_size)):
				#batch_size分の画像に対しての訓練の実行
				batch = i*batch_size
				
				
				#feed_dictでplaceholderに入れるデータを指定する
				sess.run(train_op, feed_dict={images_placeholder: train_image[batch:batch+batch_size], labels_placeholder: train_label[batch: batch+batch_size], keep_prob: 0.5})
